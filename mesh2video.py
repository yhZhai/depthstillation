...
def generate_video_from_meshes(folder_path, output_video_path):
    files = sorted([os.path.join(folder_path, file) for file in os.listdir(folder_path) if file.endswith('.obj')])
    frames = []
    ...
    for file in files:
        ...
        # Render the scene
        img = scene.show(flags={"mesh": True}, return_scene=False, resolution=[800, 600])
    ...
    video = cv2.VideoWriter(output_video_path, cv2.VideoWriter_fourcc(*'DIVX'), 15, (800, 600))
