...
parser.add_argument(
    "--num_motions", dest="num_motions", type=int, help="Number of motions", default=1
)
parser.add_argument(
    "--segment",
    dest="segment",
...
parser.add_argument(
    "--no_depth", dest="no_depth", action="store_true", help="Assumes constant depth"
)
parser.add_argument(
    "--no_sharp", dest="no_sharp", action="store_true", help="Disable depth sharpening"
)
parser.add_argument(
    "--change_k", dest="change_k", action="store_true", help="Use a different K matrix"
)
parser.add_argument(
    "--change_motion",
    dest="change_motion",
...
image_path = "samples/im0.jpg"
depth_path = "samples/d0.png"
seg_path = "samples/s0.png"
...
for idm in range(args.num_motions):
    ...
    flow_16bit = cv2.cvtColor(
        np.concatenate(
            (flow_01 * 64.0 + (2**15), np.ones_like(flow_01)[:, :, :, 0:1]), -1
        )[0],
        cv2.COLOR_BGR2RGB,
    )
    ...
    cv2.imwrite(
        os.path.join("dCOCO", "flow", "95022_%02d.png" % (idm)),
    ...
    cv2.imwrite(
        os.path.join("dCOCO", "M'", "95022_%02d.png" % (idm)), masks["M'"] * 255
    )
    cv2.imwrite(os.path.join("dCOCO", "P", "95022_%02d.png" % (idm)), masks["P"] * 255)
    cv2.imwrite(
        os.path.join("dCOCO", "H'", "95022_%02d.png" % (idm)), masks["H'"] * 255
    )
    cv2.imwrite(
        os.path.join("dCOCO", "flow_color", "95022_%02d.png" % (idm)), flow_color
    )
    plt.imsave(
        os.path.join("dCOCO", "depth_color", "95022_%02d.png" % (idm)),
        1.0 / depth[0].detach().numpy(),
        cmap="magma",
    )
    if args.segment:
        plt.imsave(
            os.path.join("dCOCO", "instances_color", "95022_%02d.png" % (idm)),
            instances_mask,
            cmap="magma",
        )
